          ...
          <div class="content-container flex flex-col p-4 gap-4">
            ...
            <div class="price-container flex items-center justify-between">
              ...
              <form method="POST" action="{{route('front.hotel.room.book', [$hotel->slug, $room->id])}}">
                @csrf
                <button type="submit" class="w-[138px] h-[48px] bg-[#4041DA] p-[12px_24px] rounded-full text-nowrap text-white font-semibold text-sm leading-[21px] flex items-center justify-center">Choose</a>
              </form>
            </div>
          </div>
        </div>
        @empty
        <p>tidak ada rooms hotel</p>
        @endforelse
      </div>
    </div>
  </section>
